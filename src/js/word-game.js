// ====================== IMPORTAÇÕES ====================== //
import { wordEasy } from "./words_game/easy.js";
import { wordNormal } from "./words_game/normal.js";
import { wordHardcore } from "./words_game/hardcore.js";
...
// ====================== FUNÇÃO AUXILIAR ====================== //
// Formatar nome -> inicial maiúscula em cada palavra
...
botoesNivel.forEach(btn => {
    btn.addEventListener("click", () => {
        ...
        // Atualizar ranking conforme nível
...
function novaPalavra(){
    ...
    // 50% de chance de começar com maiúscula
...
function encerrarJogo(){
    ...
    // Salva progresso automático caso tenha pontuação mínima
    if(pontos>=10) salvarProgresso();
}

// Checar se palavra digitada está correta
function checkMatch(){
    ...
    for(let i=0;i<letters.length;i++){
        if(i<typed.length) 
            letters[i].className = typed[i]===palavraAtual[i]?"letter correct":"letter incorrect";
        else 
    ...
    if(typed===palavraAtual){
        ...
        // A cada 10 pontos -> reinicia tempo
...
function controleJogo(){
    if(!emAndamento && !jogoEncerrado && pontos===0 && tempo===temposNivel[nivelAtual]) 
        iniciarPartida();
    else if(emAndamento){ // Pausar
        clearInterval(intervalo); 
        emAndamento=false; 
        input.disabled=true;
        btnControle.textContent="Retomar"; 
        status.textContent="Pausado";
    }
    else if(!emAndamento && !jogoEncerrado){ // Retomar
        emAndamento=true; 
        input.disabled=false; 
        btnControle.textContent="Pausar"; 
        status.textContent="Jogando...";
        iniciarTimer(); 
        input.focus();
        novaPalavra();
    }
...
// Salvar manualmente
btnSalvar.addEventListener("click",()=>{ if(emAndamento == false && pontos>=10 && tempo <= 1) salvarProgresso(); });

// ====================== SALVAR PROGRESSO ====================== //
async function salvarProgresso(){
    const { doc, setDoc, getDoc } = window.firebaseTools;

    // Caixa para pedir nome e código
...
async function listarJogadores(nivelFiltro="Normal"){
    ...
    snapshot.forEach(doc=>{
        const data=doc.data();
        if(data.dificuldade===nivelFiltro) 
            lista.push({nome:data.nome||"Sem nome", score:data.score||0});
    });
    ...
    topLi.forEach((li,idx)=>{
        ...
        if(jogador){
            li.querySelector(".name").textContent = formatarNome(jogador.nome);
            li.querySelector(".score").textContent = jogador.score;
        } else {
            li.querySelector(".name").textContent = "-";
            li.querySelector(".score").textContent = "-";
        }
    });
    ...
    ordenada.forEach((jogador,idx)=>{
        ...
        tr.innerHTML=`<td data-label="#">${idx+1}</td>
                        <td data-label="Nome">${formatarNome(jogador.nome)}</td>
                        <td data-label="Pontos">${jogador.score}</td>`;
        tbody.appendChild(tr);
    });
}

// ====================== INICIALIZA RANKING PADRÃO ====================== //
